...
test('approves proposed records and verifies the result', async () => {
  ...
  const fetchImpl = async (_url, options = {}) => {
    ...
    reads += 1;
    if (reads === 1) return jsonResponse([{ id: 'review-usa', status: 'pending', suggested_record: { id: 'approved-usa', noc: 'USA' }, confirmation_record: null }]);
    if (reads === 2) return jsonResponse([]);
    if (reads === 3) return jsonResponse([{ id: 'review-usa', status: 'approved', confirmation_record: { id: 'approved-usa', noc: 'USA' } }]);
    return jsonResponse([]);
  };
  ...
  assert.deepEqual(result, { approvedCount: 1, alreadyApprovedCount: 0, verifiedCount: 1 });
  assert.equal(patches.length, 1);
  assert.equal(patches[0].status, 'approved');
  assert.deepEqual(patches[0].confirmation_record, { id: 'approved-usa', noc: 'USA' });
});
